...
export function AgentNode({ agentId, state, activeTool, stackMode = false, stepNumber }: AgentNodeProps) {
        ...
        <div className="flex items-start justify-between mb-3">
          <div className="flex items-center gap-3">
            {typeof stepNumber === 'number' && (
              <div className={`flex h-9 w-9 items-center justify-center rounded-xl border text-sm font-black ${
                stackMode
                  ? 'border-[var(--color-border)] bg-[var(--color-soft)] text-[var(--color-muted)]'
                  : 'border-[var(--color-border)] bg-[var(--color-soft)] text-[var(--color-text)]'
              }`}>
                {stepNumber}
              </div>
            )}
